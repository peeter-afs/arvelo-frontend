...
/** Newest import per draft wins, so a replaced original shows up immediately. */
function importsByDraft(items: PurchaseInvoiceImportListItem[]) { const map = new Map<string, PurchaseInvoiceImportListItem>(); [...items].sort((a, b) => +new Date(a.created_at) - +new Date(b.created_at)).forEach((r) => { if (r.draft_invoice_id && r.document_id) map.set(r.draft_invoice_id, r); }); return map; }
...
export default function PurchaseInvoiceWorkspace() {
  const rootRef = useRef<HTMLDivElement>(null); const searchRef = useRef<HTMLInputElement>(null); const rowRefs = useRef(new Map<string, HTMLDivElement>()); const fileRef = useRef<HTMLInputElement>(null); const uploadTarget = useRef<InvoiceListItem | null>(null); const [pdfBox, setPdfBox] = useState<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    importApi.listPurchaseInvoiceImports({ limit: 500 }).then((res) => setImports(importsByDraft(res.items))).catch(() => {});
  ...
  const ctx: Ctx = { partnerMap, memberMap, accountMap, ccMap, prjMap, batches, imports, payments, reminders, supplierIban, receiptSettings, action, docUrls, zoom, zoomValue, fitZoom, pdfPages, ptab, wide, visible, selectedId, setPtab, setZoom, stepZoom, setPdfPages, onNavigate: selectRelative, onWide: () => setWide((v) => !v), onOpen: () => setMode('open'), onClose: () => setMode('list'), onJournal: (inv) => void openJournal(inv), submit, approve, reject, remindNow, noDoc, upload: (inv) => { uploadTarget.current = inv || null; if (fileRef.current) { fileRef.current.accept = '.pdf'; fileRef.current.click(); } }, pay: (ids) => setModal({ kind: 'pay', ids }), downloadDoc: (inv) => { const d = docFor(inv); const u = d?.document_id ? docUrls[d.document_id] : null; if (u) { const a = document.createElement('a'); a.href = u; a.download = d?.file_name || 'originaal'; a.click(); } } };
...
type Ctx = {
  ...
  submit: (inv: InvoiceListItem) => Promise<void>; approve: (inv: InvoiceListItem) => Promise<void>; reject: (inv: InvoiceListItem) => void; remindNow: (inv: InvoiceListItem) => Promise<void>; noDoc: (inv: InvoiceListItem) => Promise<void>; upload: (inv?: InvoiceListItem) => void; pay: (ids: string[]) => void; downloadDoc: (inv: InvoiceListItem) => void;
};
...
function PdfPane({ inv, ctx, seg, lineCount, setPdfBox }: { inv: InvoiceListItem; ctx: Ctx; seg: boolean; lineCount: number; setPdfBox: (el: HTMLDivElement | null) => void }) {
  ...
  if (!doc) return <div className={styles.pview}><div className={styles.noorig}>
    ...
    <div className={styles.noorigActs}><button className={`${styles.button} ${styles.primary}`} disabled={!!ctx.action} onClick={() => ctx.upload(inv)}>Laadi originaal üles</button>{bank && <><button className={styles.button} disabled={!!ctx.action} onClick={() => void ctx.remindNow(inv)}>Saada meeldetuletus kohe</button><button className={`${styles.button} ${styles.ghost}`} disabled={!!ctx.action || inv.receipt_reminder_state !== 'active'} onClick={() => void ctx.noDoc(inv)}>Originaali ei tule</button></>}</div>
