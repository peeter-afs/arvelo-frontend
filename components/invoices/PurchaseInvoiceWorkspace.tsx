...
export default function PurchaseInvoiceWorkspace() {
  ...
  const [costCenters, setCostCenters] = useState<CostCenter[]>([]); const [projects, setProjects] = useState<Project[]>([]);
  ...
  const remindNow = (inv: InvoiceListItem) => run(`remind:${inv.id}`, async () => { await bankingApi.sendMissingReceiptRemindersNow(); await loadInvoices(inv.id); showToast.success(`Meeldetuletus saadetud · ${receiptSettings?.responsible_email || 'vastutajale'}`); });
...
type Ctx = {
  ...
  batches: Map<string, BatchRef>; imports: Map<string, PurchaseInvoiceImportListItem>; payments: PaymentListItem[]; supplierIban: Record<string, SupplierBankAccount | null>; receiptSettings: MissingReceiptSettings | null; action: string | null;
...
const reminderDates = (inv: InvoiceListItem, ctx: Ctx) => { const n = Number(inv.receipt_reminder_sent_count || 0); const last = inv.receipt_reminder_last_sent_at ? new Date(inv.receipt_reminder_last_sent_at) : null; const freq = Number(ctx.receiptSettings?.frequency_days || 3); const dates: Date[] = []; if (last) for (let i = n - 1; i >= 0; i -= 1) dates.push(new Date(last.getTime() - i * freq * DAY)); const stopped = inv.receipt_reminder_state === 'dismissed' || !!inv.receipt_dismissed_reason || (ctx.receiptSettings?.max_reminders != null && n >= Number(ctx.receiptSettings.max_reminders)); const next = stopped ? null : new Date(Math.max(Date.now() + DAY, (last ? last.getTime() : new Date(inv.created_at).getTime()) + freq * DAY)); return { dates, next, to: ctx.receiptSettings?.responsible_email || '' }; };
...
function ApprovalTimeline({ inv, ctx }: { inv: InvoiceListItem; ctx: Ctx }) {
  ...
  if (k === 'draft') { rem.dates.forEach((d, i) => rows.push({ d, t: `Meeldetuletus ${i + 1} saadetud · ${rem.to}`, s: 'wait', title: `Saadetud ${rem.to}` })); if (srcKey(inv) === 'bank' && rem.next) rows.push({ d: rem.next, t: 'Järgmine meeldetuletus (automaatne)' }); rows.push({ d: null, t: doc || srcKey(inv) !== 'bank' ? 'Täienda ja saada kinnitamiseks' : 'Ootab originaali', s: 'wait' }); }
